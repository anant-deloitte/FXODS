/* Transformation Name ==>Oci_Etl_All_Rejection ,Transformation Type ==>Target 

{{
	config(
		materialized='incremental',
		alias='ETL_ALL_REJECTION',
		schema='',
...
{{ config(
    materialized='table', 
    schema='ITSS_AP',    
    alias='j_R12_301_PFL_AP_EXPENSE_REPORT_DS_Ld_Oci_Etl_All_Rejection'
) }}
...
DSfilr_Lnk_Xfm_To_Etl_All_Reject_2Out as (
	Select 
	SvarReject,
	 SvarCODE,
	 SvarREPORT,
	 ...
	 REPROCESS_COUNTER,
	 SRLNO 
From DSstgVar_Xfm_ColumnsOut as DSstgVar_Xfm_ColumnsOut Where SvarReject = 1 AND REPROCESS_COUNTER <= '{{ var("p_Reprocess_Record_Threshold") }}' ),

Lnk_Xfm_To_Etl_All_Reject_2Out as (
	Select 
	'{{ var("p_Staging_Table_Name") }}' as STAGING_TABLE_NAME,
	 'ITSSLIVE' as SRC_SYS_CD,
	 TABLE_TXN_SEQ_NO as TABLE_TXN_SEQ_NO,
	 DB_TXN_SEQ_NO as DB_TXN_SEQ_NO,
	 1 as REJECT_REASON_CD,
	 'Referential Integrity Failure For REPORT_LINE_ID=' || REPORT_LINE_ID || ' Missing ' || SvarCODE || SvarREPORT || SvarORG || SvarORGANIZATION as REJ_DESCRIPTION,
	 TO_DECIMAL ( '{{ var("p_Batch_ID") }}' ,38,0) as BATCH_ID,
	 1 as SRC_SYS_ID,
	 CURRENT_TIMESTAMP as ERR_TIMESTAMP,
	 '{{ var("p_Performance_Table_Name") }}' as PFL_TABLE_NAME,
	 REPROCESS_COUNTER as REPROCESS_COUNTER 
From DSfilr_Lnk_Xfm_To_Etl_All_Reject_2Out as DSfilr_Lnk_Xfm_To_Etl_All_Reject_2Out)



Select *
	 
From Lnk_Xfm_To_Etl_All_Reject_2Out as Lnk_Xfm_To_Etl_All_Reject_2Out