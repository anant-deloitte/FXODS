...
*/
-- Truncate and create table
{{ config(
    materialized='table', 
    schema='ITSS_AP',    
    alias='j_R12_301_PFL_AP_EXPENSE_REPORT_DS_Ld_DS_Pfl_Expense_Report'
) }}
...
SRLNO
 from 
--(SELECT mainq.* FROM ( SELECT taerl.report_line_id AS report_line_id, taerl.distribution_line_number AS distribution_line_number, taerl.amount AS amount, taerl.currency_code AS currency_code, taerl.receipt_currency_amount AS receipt_currency_amount, taerl.daily_amount AS daily_amount, taerl.web_parameter_id AS web_parameter_id, taerl.category_code AS category_code, taerl.submitted_amount AS submitted_amount, taerl.org_id AS org_id, taerl.itemization_parent_id AS itemization_parent_id, taerl.table_txn_seq_no AS table_txn_seq_no, taerl.db_txn_seq_no AS db_txn_seq_no, taerl.operation AS operation, taerl.batch_id AS batch_id, aerh.report_header_id AS report_header_id, aerh.employee_id AS employee_id , aerh.week_end_date AS week_end_date, aerh.vendor_site_id AS exp_header_vendor_site_id, aerh.invoice_num AS invoice_num, aerh.set_of_books_id AS ledger_id, aerh.source AS source , aerh.maximum_amount_to_apply AS maximum_amount_to_apply, aerh.prepay_num AS prepay_num, aerh.paid_on_behalf_employee_id AS paid_on_behalf_employee_id, gcc.code_combination_id AS code_combination_id, gcc.segment1 AS expense_line_co_code, gcc.segment2 AS expense_line_cost_centre, gcc.segment3 AS expense_line_nominal_account, gcc.segment4 AS expense_line_branch_code, gcc.segment5 AS expense_line_product_code, gcc.segment1 AS co_code, aerh.employee_id AS person_id, pap.employee_number AS employee_number, pap.full_name AS full_name, aerh.vendor_site_id AS ap_exp_header_supp_site_id, (SELECT max (CASE WHEN vendor_site_code IN ('OFFICE','HOME') THEN 'Y' ELSE ' ' END) office_home_flag FROM ITSS_AP.ods_per_all_people_f a LEFT JOIN  ITSS_AP.ods_ap_suppliers b ON  a.person_id = b.employee_id LEFT JOIN ITSS_AP.ods_ap_supplier_sites_all c ON b.vendor_id = c.vendor_id WHERE (a.effective_end_date = to_date('31-Dec-4712' ,'DD-Mon-YYYY') OR a.effective_end_date IS NULL ) AND a.person_id=aerh.employee_id AND c.org_id=aerh.org_id ) office_home_flag, aerh.report_header_id AS report_header_id_key, aerh.org_id AS org_id_key, gcc.code_combination_id AS code_combination_id_key, hou.organization_id AS organization_id_key, taerl.reprocess_counter AS reprocess_counter, row_number () OVER (PARTITION BY taerl.report_line_id ORDER BY db_txn_seq_no DESC NULLS LAST) srlno FROM  {{ ref('j_R12_300_Stg_PFL_EXPENSE_REPORT_DB_Ld') }}  taerl LEFT JOIN ITSS_AP.ods_ap_expense_rpt_headers_all aerh ON taerl.report_header_id = aerh.report_header_id AND (aerh.source <> 'NonValidatedWebExpense' OR aerh.workflow_approved_flag IS NULL) AND aerh.source <> 'Both Pay' AND taerl.org_id = aerh.org_id LEFT JOIN ITSS_AP.ods_gl_code_combinations gcc ON taerl.code_combination_id = gcc.code_combination_id LEFT JOIN ITSS_AP.ods_hr_operating_units hou ON hou.organization_id = aerh.org_id LEFT JOIN ITSS_AP.ods_per_all_people_f pap ON aerh.employee_id = pap.person_id AND (pap.effective_end_date = to_date ('31-Dec-4712' ,'DD-Mon-YYYY') OR pap.effective_end_date IS NULL ) WHERE NOT EXISTS (SELECT aerp.parameter_id FROM ITSS_AP.ods_ap_expense_rpt_params_all aerp, ITSS_AP.ods_ap_expense_reports_all aer WHERE aer.report_type = 'Seeded Personal Expense' AND aerp.expense_type_code = 'PERSONAL' AND aer.expense_report_id = aerp.expense_report_id AND taerl.web_parameter_id = aerp.parameter_id) AND ((taerl.itemization_parent_id IS NULL) OR (taerl.itemization_parent_id <> -1) ) ) mainq where srlno = 1)
--(SELECT mainq.* FROM ( SELECT taerl.report_line_id AS report_line_id, taerl.distribution_line_number AS distribution_line_number, taerl.amount AS amount, taerl.currency_code AS currency_code, taerl.receipt_currency_amount AS receipt_currency_amount, taerl.daily_amount AS daily_amount, taerl.web_parameter_id AS web_parameter_id, taerl.category_code AS category_code, taerl.submitted_amount AS submitted_amount, taerl.org_id AS org_id, taerl.itemization_parent_id AS itemization_parent_id, taerl.table_txn_seq_no AS table_txn_seq_no, taerl.db_txn_seq_no AS db_txn_seq_no, taerl.operation AS operation, taerl.batch_id AS batch_id, aerh.report_header_id AS report_header_id, aerh.employee_id AS employee_id , aerh.week_end_date AS week_end_date, aerh.vendor_site_id AS exp_header_vendor_site_id, aerh.invoice_num AS invoice_num, aerh.set_of_books_id AS ledger_id, aerh.source AS source , aerh.maximum_amount_to_apply AS maximum_amount_to_apply, aerh.prepay_num AS prepay_num, aerh.paid_on_behalf_employee_id AS paid_on_behalf_employee_id, gcc.code_combination_id AS code_combination_id, gcc.segment1 AS expense_line_co_code, gcc.segment2 AS expense_line_cost_centre, gcc.segment3 AS expense_line_nominal_account, gcc.segment4 AS expense_line_branch_code, gcc.segment5 AS expense_line_product_code, gcc.segment1 AS co_code, aerh.employee_id AS person_id, pap.employee_number AS employee_number, pap.full_name AS full_name, aerh.vendor_site_id AS ap_exp_header_supp_site_id, (SELECT max (CASE WHEN vendor_site_code IN ('OFFICE','HOME') THEN 'Y' ELSE ' ' END) office_home_flag FROM ODS.ITSS_AP.ods_per_all_people_f a LEFT JOIN  ODS.ITSS_AP.ods_ap_suppliers b ON  a.person_id = b.employee_id LEFT JOIN ODS.ITSS_AP.ods_ap_supplier_sites_all c ON b.vendor_id = c.vendor_id WHERE (a.effective_end_date = to_date('31-Dec-4712' ,'DD-Mon-YYYY') OR a.effective_end_date IS NULL ) AND a.person_id=aerh.employee_id AND c.org_id=aerh.org_id ) office_home_flag, aerh.report_header_id AS report_header_id_key, aerh.org_id AS org_id_key, gcc.code_combination_id AS code_combination_id_key, hou.organization_id AS organization_id_key, taerl.reprocess_counter AS reprocess_counter, row_number () OVER (PARTITION BY taerl.report_line_id ORDER BY db_txn_seq_no DESC NULLS LAST) srlno FROM ODS.ITSS_AP.tmp_ods_ap_exp_rpt_lines_all taerl LEFT JOIN ODS.ITSS_AP.ods_ap_expense_rpt_headers_all aerh ON taerl.report_header_id = aerh.report_header_id AND (aerh.source <> 'NonValidatedWebExpense' OR aerh.workflow_approved_flag IS NULL) AND aerh.source <> 'Both Pay' AND taerl.org_id = aerh.org_id LEFT JOIN ODS.ITSS_AP.ods_gl_code_combinations gcc ON taerl.code_combination_id = gcc.code_combination_id LEFT JOIN ODS.ITSS_AP.ods_hr_operating_units hou ON hou.organization_id = aerh.org_id LEFT JOIN ODS.ITSS_AP.ods_per_all_people_f pap ON aerh.employee_id = pap.person_id AND (pap.effective_end_date = to_date ('31-Dec-4712' ,'DD-Mon-YYYY') OR pap.effective_end_date IS NULL ) WHERE NOT EXISTS (SELECT aerp.parameter_id FROM ODS.ITSS_AP.ods_ap_expense_rpt_params_all aerp, ODS.ITSS_AP.ods_ap_expense_reports_all aer WHERE aer.report_type = 'Seeded Personal Expense' AND aerp.expense_type_code = 'PERSONAL' AND aer.expense_report_id = aerp.expense_report_id AND taerl.web_parameter_id = aerp.parameter_id) AND ((taerl.itemization_parent_id IS NULL) OR (taerl.itemization_parent_id <> -1) ) ) mainq where srlno = 1)
--(SELECT mainq.* FROM ( SELECT taerl.report_line_id AS report_line_id, taerl.distribution_line_number AS distribution_line_number, taerl.amount AS amount, taerl.currency_code AS currency_code, taerl.receipt_currency_amount AS receipt_currency_amount, taerl.daily_amount AS daily_amount, taerl.web_parameter_id AS web_parameter_id, taerl.category_code AS category_code, taerl.submitted_amount AS submitted_amount, taerl.org_id AS org_id, taerl.itemization_parent_id AS itemization_parent_id, taerl.table_txn_seq_no AS table_txn_seq_no, taerl.db_txn_seq_no AS db_txn_seq_no, taerl.operation AS operation, taerl.batch_id AS batch_id, aerh.report_header_id AS report_header_id, aerh.employee_id AS employee_id , aerh.week_end_date AS week_end_date, aerh.vendor_site_id AS exp_header_vendor_site_id, aerh.invoice_num AS invoice_num, aerh.set_of_books_id AS ledger_id, aerh.source AS source , aerh.maximum_amount_to_apply AS maximum_amount_to_apply, aerh.prepay_num AS prepay_num, aerh.paid_on_behalf_employee_id AS paid_on_behalf_employee_id, gcc.code_combination_id AS code_combination_id, gcc.segment1 AS expense_line_co_code, gcc.segment2 AS expense_line_cost_centre, gcc.segment3 AS expense_line_nominal_account, gcc.segment4 AS expense_line_branch_code, gcc.segment5 AS expense_line_product_code, gcc.segment1 AS co_code, aerh.employee_id AS person_id, pap.employee_number AS employee_number, pap.full_name AS full_name, aerh.vendor_site_id AS ap_exp_header_supp_site_id, (SELECT max (CASE WHEN vendor_site_code IN ('OFFICE','HOME') THEN 'Y' ELSE ' ' END) office_home_flag FROM ITSS_AP.ods_per_all_people_f a LEFT JOIN  ITSS_AP.ods_ap_suppliers b ON  a.person_id = b.employee_id LEFT JOIN ITSS_AP.ods_ap_supplier_sites_all c ON b.vendor_id = c.vendor_id WHERE (a.effective_end_date = to_date('31-Dec-4712' ,'DD-Mon-YYYY') OR a.effective_end_date IS NULL ) AND a.person_id=aerh.employee_id AND c.org_id=aerh.org_id ) office_home_flag, aerh.report_header_id AS report_header_id_key, aerh.org_id AS org_id_key, gcc.code_combination_id AS code_combination_id_key, hou.organization_id AS organization_id_key, taerl.reprocess_counter AS reprocess_counter, row_number () OVER (PARTITION BY taerl.report_line_id ORDER BY db_txn_seq_no DESC NULLS LAST) srlno FROM ITSS_AP.tmp_ods_ap_exp_rpt_lines_all taerl LEFT JOIN ITSS_AP.ods_ap_expense_rpt_headers_all aerh ON taerl.report_header_id = aerh.report_header_id AND (aerh.source <> 'NonValidatedWebExpense' OR aerh.workflow_approved_flag IS NULL) AND aerh.source <> 'Both Pay' AND taerl.org_id = aerh.org_id LEFT JOIN ITSS_AP.ods_gl_code_combinations gcc ON taerl.code_combination_id = gcc.code_combination_id LEFT JOIN ITSS_AP.ods_hr_operating_units hou ON hou.organization_id = aerh.org_id LEFT JOIN ITSS_AP.ods_per_all_people_f pap ON aerh.employee_id = pap.person_id AND (pap.effective_end_date = to_date ('31-Dec-4712' ,'DD-Mon-YYYY') OR pap.effective_end_date IS NULL ) WHERE NOT EXISTS (SELECT aerp.parameter_id FROM ITSS_AP.ods_ap_expense_rpt_params_all aerp, ITSS_AP.ods_ap_expense_reports_all aer WHERE aer.report_type = 'Seeded Personal Expense' AND aerp.expense_type_code = 'PERSONAL' AND aer.expense_report_id = aerp.expense_report_id AND taerl.web_parameter_id = aerp.parameter_id) AND ((taerl.itemization_parent_id IS NULL) OR (taerl.itemization_parent_id <> -1) ) ) mainq where srlno = 1)  --working
--(SELECT mainq.* FROM ( SELECT taerl.report_line_id AS report_line_id, taerl.distribution_line_number AS distribution_line_number, taerl.amount AS amount, taerl.currency_code AS currency_code, taerl.receipt_currency_amount AS receipt_currency_amount, taerl.daily_amount AS daily_amount, taerl.web_parameter_id AS web_parameter_id, taerl.category_code AS category_code, taerl.submitted_amount AS submitted_amount, taerl.org_id AS org_id, taerl.itemization_parent_id AS itemization_parent_id, taerl.table_txn_seq_no AS table_txn_seq_no, taerl.db_txn_seq_no AS db_txn_seq_no, taerl.operation AS operation, taerl.batch_id AS batch_id, aerh.report_header_id AS report_header_id, aerh.employee_id AS employee_id , aerh.week_end_date AS week_end_date, aerh.vendor_site_id AS exp_header_vendor_site_id, aerh.invoice_num AS invoice_num, aerh.set_of_books_id AS ledger_id, aerh.source AS source , aerh.maximum_amount_to_apply AS maximum_amount_to_apply, aerh.prepay_num AS prepay_num, aerh.paid_on_behalf_employee_id AS paid_on_behalf_employee_id, gcc.code_combination_id AS code_combination_id, gcc.segment1 AS expense_line_co_code, gcc.segment2 AS expense_line_cost_centre, gcc.segment3 AS expense_line_nominal_account, gcc.segment4 AS expense_line_branch_code, gcc.segment5 AS expense_line_product_code, gcc.segment1 AS co_code, aerh.employee_id AS person_id, pap.employee_number AS employee_number, pap.full_name AS full_name, aerh.vendor_site_id AS ap_exp_header_supp_site_id, (SELECT max (CASE WHEN vendor_site_code IN ('OFFICE','HOME') THEN 'Y' ELSE ' ' END) office_home_flag FROM ITSS_AP.ods_per_all_people_f a LEFT JOIN  ITSS_AP.ods_ap_suppliers b ON  a.person_id = b.employee_id LEFT JOIN ITSS_AP.ods_ap_supplier_sites_all c ON b.vendor_id = c.vendor_id WHERE (a.effective_end_date = to_date('31-Dec-4712' ,'DD-Mon-YYYY') OR a.effective_end_date IS NULL ) AND a.person_id=aerh.employee_id AND c.org_id=aerh.org_id ) office_home_flag, aerh.report_header_id AS report_header_id_key, aerh.org_id AS org_id_key, gcc.code_combination_id AS code_combination_id_key, hou.organization_id AS organization_id_key, taerl.reprocess_counter AS reprocess_counter, row_number () OVER (PARTITION BY taerl.report_line_id ORDER BY db_txn_seq_no DESC NULLS LAST) srlno FROM {{ ref('j_R12_300_Stg_PFL_EXPENSE_REPORT_DB_Ld') }} taerl LEFT JOIN ITSS_AP.ods_ap_expense_rpt_headers_all aerh ON taerl.report_header_id = aerh.report_header_id AND (aerh.source <> 'NonValidatedWebExpense' OR aerh.workflow_approved_flag IS NULL) AND aerh.source <> 'Both Pay' AND taerl.org_id = aerh.org_id LEFT JOIN ITSS_AP.ods_gl_code_combinations gcc ON taerl.code_combination_id = gcc.code_combination_id LEFT JOIN ITSS_AP.ods_hr_operating_units hou ON hou.organization_id = aerh.org_id LEFT JOIN ITSS_AP.ods_per_all_people_f pap ON aerh.employee_id = pap.person_id AND (pap.effective_end_date = to_date ('31-Dec-4712' ,'DD-Mon-YYYY') OR pap.effective_end_date IS NULL ) WHERE NOT EXISTS (SELECT aerp.parameter_id FROM ITSS_AP.ods_ap_expense_rpt_params_all aerp, ITSS_AP.ods_ap_expense_reports_all aer WHERE aer.report_type = 'Seeded Personal Expense' AND aerp.expense_type_code = 'PERSONAL' AND aer.expense_report_id = aerp.expense_report_id AND taerl.web_parameter_id = aerp.parameter_id) AND ((taerl.itemization_parent_id IS NULL) OR (taerl.itemization_parent_id <> -1) ) ) mainq where srlno = 1)
--(SELECT mainq.* FROM ( SELECT taerl.report_line_id AS report_line_id, taerl.distribution_line_number AS distribution_line_number, taerl.amount AS amount, taerl.currency_code AS currency_code, taerl.receipt_currency_amount AS receipt_currency_amount, taerl.daily_amount AS daily_amount, taerl.web_parameter_id AS web_parameter_id, taerl.category_code AS category_code, taerl.submitted_amount AS submitted_amount, taerl.org_id AS org_id, taerl.itemization_parent_id AS itemization_parent_id, taerl.table_txn_seq_no AS table_txn_seq_no, taerl.db_txn_seq_no AS db_txn_seq_no, taerl.operation AS operation, taerl.batch_id AS batch_id, aerh.report_header_id AS report_header_id, aerh.employee_id AS employee_id , aerh.week_end_date AS week_end_date, aerh.vendor_site_id AS exp_header_vendor_site_id, aerh.invoice_num AS invoice_num, aerh.set_of_books_id AS ledger_id, aerh.source AS source , aerh.maximum_amount_to_apply AS maximum_amount_to_apply, aerh.prepay_num AS prepay_num, aerh.paid_on_behalf_employee_id AS paid_on_behalf_employee_id, gcc.code_combination_id AS code_combination_id, gcc.segment1 AS expense_line_co_code, gcc.segment2 AS expense_line_cost_centre, gcc.segment3 AS expense_line_nominal_account, gcc.segment4 AS expense_line_branch_code, gcc.segment5 AS expense_line_product_code, gcc.segment1 AS co_code, aerh.employee_id AS person_id, pap.employee_number AS employee_number, pap.full_name AS full_name, aerh.vendor_site_id AS ap_exp_header_supp_site_id, (SELECT max (CASE WHEN vendor_site_code IN ('OFFICE','HOME') THEN 'Y' ELSE ' ' END) office_home_flag FROM ITSS_AP.ods_per_all_people_f a LEFT JOIN  ITSS_AP.ods_ap_suppliers b ON  a.person_id = b.employee_id LEFT JOIN ITSS_AP.ods_ap_supplier_sites_all c ON b.vendor_id = c.vendor_id WHERE (a.effective_end_date = to_date('31-Dec-4712' ,'DD-Mon-YYYY') OR a.effective_end_date IS NULL ) AND a.person_id=aerh.employee_id AND c.org_id=aerh.org_id ) office_home_flag, aerh.report_header_id AS report_header_id_key, aerh.org_id AS org_id_key, gcc.code_combination_id AS code_combination_id_key, hou.organization_id AS organization_id_key, taerl.reprocess_counter AS reprocess_counter, row_number () OVER (PARTITION BY taerl.report_line_id ORDER BY db_txn_seq_no DESC NULLS LAST) srlno FROM  FXODS_MODEL.J_R12_300_STG_PFL_EXPENSE_REPORT_DB_LD  taerl LEFT JOIN ITSS_AP.ods_ap_expense_rpt_headers_all aerh ON taerl.report_header_id = aerh.report_header_id AND (aerh.source <> 'NonValidatedWebExpense' OR aerh.workflow_approved_flag IS NULL) AND aerh.source <> 'Both Pay' AND taerl.org_id = aerh.org_id LEFT JOIN ITSS_AP.ods_gl_code_combinations gcc ON taerl.code_combination_id = gcc.code_combination_id LEFT JOIN ITSS_AP.ods_hr_operating_units hou ON hou.organization_id = aerh.org_id LEFT JOIN ITSS_AP.ods_per_all_people_f pap ON aerh.employee_id = pap.person_id AND (pap.effective_end_date = to_date ('31-Dec-4712' ,'DD-Mon-YYYY') OR pap.effective_end_date IS NULL ) WHERE NOT EXISTS (SELECT aerp.parameter_id FROM ITSS_AP.ods_ap_expense_rpt_params_all aerp, ITSS_AP.ods_ap_expense_reports_all aer WHERE aer.report_type = 'Seeded Personal Expense' AND aerp.expense_type_code = 'PERSONAL' AND aer.expense_report_id = aerp.expense_report_id AND taerl.web_parameter_id = aerp.parameter_id) AND ((taerl.itemization_parent_id IS NULL) OR (taerl.itemization_parent_id <> -1) ) ) mainq where srlno = 1)
--(SELECT mainq.* FROM ( SELECT taerl.report_line_id AS report_line_id, taerl.distribution_line_number AS distribution_line_number, taerl.amount AS amount, taerl.currency_code AS currency_code, taerl.receipt_currency_amount AS receipt_currency_amount, taerl.daily_amount AS daily_amount, taerl.web_parameter_id AS web_parameter_id, taerl.category_code AS category_code, taerl.submitted_amount AS submitted_amount, taerl.org_id AS org_id, taerl.itemization_parent_id AS itemization_parent_id, taerl.table_txn_seq_no AS table_txn_seq_no, taerl.db_txn_seq_no AS db_txn_seq_no, taerl.operation AS operation, taerl.batch_id AS batch_id, aerh.report_header_id AS report_header_id, aerh.employee_id AS employee_id , aerh.week_end_date AS week_end_date, aerh.vendor_site_id AS exp_header_vendor_site_id, aerh.invoice_num AS invoice_num, aerh.set_of_books_id AS ledger_id, aerh.source AS source , aerh.maximum_amount_to_apply AS maximum_amount_to_apply, aerh.prepay_num AS prepay_num, aerh.paid_on_behalf_employee_id AS paid_on_behalf_employee_id, gcc.code_combination_id AS code_combination_id, gcc.segment1 AS expense_line_co_code, gcc.segment2 AS expense_line_cost_centre, gcc.segment3 AS expense_line_nominal_account, gcc.segment4 AS expense_line_branch_code, gcc.segment5 AS expense_line_product_code, gcc.segment1 AS co_code, aerh.employee_id AS person_id, pap.employee_number AS employee_number, pap.full_name AS full_name, aerh.vendor_site_id AS ap_exp_header_supp_site_id, (SELECT max (CASE WHEN vendor_site_code IN ('OFFICE','HOME') THEN 'Y' ELSE ' ' END) office_home_flag FROM ITSS_AP.ods_per_all_people_f a LEFT JOIN  ITSS_AP.ods_ap_suppliers b ON  a.person_id = b.employee_id LEFT JOIN ITSS_AP.ods_ap_supplier_sites_all c ON b.vendor_id = c.vendor_id WHERE (a.effective_end_date = to_date('31-Dec-4712' ,'DD-Mon-YYYY') OR a.effective_end_date IS NULL ) AND a.person_id=aerh.employee_id AND c.org_id=aerh.org_id ) office_home_flag, aerh.report_header_id AS report_header_id_key, aerh.org_id AS org_id_key, gcc.code_combination_id AS code_combination_id_key, hou.organization_id AS organization_id_key, taerl.reprocess_counter AS reprocess_counter, row_number () OVER (PARTITION BY taerl.report_line_id ORDER BY db_txn_seq_no DESC NULLS LAST) srlno FROM  {{ ref('j_R12_300_Stg_PFL_EXPENSE_REPORT_DB_Ld') }}  taerl LEFT JOIN ITSS_AP.ods_ap_expense_rpt_headers_all aerh ON taerl.report_header_id = aerh.report_header_id AND (aerh.source <> 'NonValidatedWebExpense' OR aerh.workflow_approved_flag IS NULL) AND aerh.source <> 'Both Pay' AND taerl.org_id = aerh.org_id LEFT JOIN ITSS_AP.ods_gl_code_combinations gcc ON taerl.code_combination_id = gcc.code_combination_id LEFT JOIN ITSS_AP.ods_hr_operating_units hou ON hou.organization_id = aerh.org_id LEFT JOIN ITSS_AP.ods_per_all_people_f pap ON aerh.employee_id = pap.person_id AND (pap.effective_end_date = to_date ('31-Dec-4712' ,'DD-Mon-YYYY') OR pap.effective_end_date IS NULL ) WHERE NOT EXISTS (SELECT aerp.parameter_id FROM ITSS_AP.ods_ap_expense_rpt_params_all aerp, ITSS_AP.ods_ap_expense_reports_all aer WHERE aer.report_type = 'Seeded Personal Expense' AND aerp.expense_type_code = 'PERSONAL' AND aer.expense_report_id = aerp.expense_report_id AND taerl.web_parameter_id = aerp.parameter_id) AND ((taerl.itemization_parent_id IS NULL) OR (taerl.itemization_parent_id <> -1) ) ) mainq where srlno = 1)
(SELECT mainq.* FROM ( SELECT taerl.report_line_id AS report_line_id, taerl.distribution_line_number AS distribution_line_number, taerl.amount AS amount, taerl.currency_code AS currency_code, taerl.receipt_currency_amount AS receipt_currency_amount, taerl.daily_amount AS daily_amount, taerl.web_parameter_id AS web_parameter_id, taerl.category_code AS category_code, taerl.submitted_amount AS submitted_amount, taerl.org_id AS org_id, taerl.itemization_parent_id AS itemization_parent_id, taerl.table_txn_seq_no AS table_txn_seq_no, taerl.db_txn_seq_no AS db_txn_seq_no, taerl.operation AS operation, taerl.batch_id AS batch_id, aerh.report_header_id AS report_header_id, aerh.employee_id AS employee_id , aerh.week_end_date AS week_end_date, aerh.vendor_site_id AS exp_header_vendor_site_id, aerh.invoice_num AS invoice_num, aerh.set_of_books_id AS ledger_id, aerh.source AS source , aerh.maximum_amount_to_apply AS maximum_amount_to_apply, aerh.prepay_num AS prepay_num, aerh.paid_on_behalf_employee_id AS paid_on_behalf_employee_id, gcc.code_combination_id AS code_combination_id, gcc.segment1 AS expense_line_co_code, gcc.segment2 AS expense_line_cost_centre, gcc.segment3 AS expense_line_nominal_account, gcc.segment4 AS expense_line_branch_code, gcc.segment5 AS expense_line_product_code, gcc.segment1 AS co_code, aerh.employee_id AS person_id, pap.employee_number AS employee_number, pap.full_name AS full_name, aerh.vendor_site_id AS ap_exp_header_supp_site_id, (SELECT max (CASE WHEN vendor_site_code IN ('OFFICE','HOME') THEN 'Y' ELSE ' ' END) office_home_flag FROM ITSS_AP.ods_per_all_people_f a LEFT JOIN  ITSS_AP.ods_ap_suppliers b ON  a.person_id = b.employee_id LEFT JOIN ITSS_AP.ods_ap_supplier_sites_all c ON b.vendor_id = c.vendor_id WHERE (a.effective_end_date = to_date('31-Dec-4712' ,'DD-Mon-YYYY') OR a.effective_end_date IS NULL ) AND a.person_id=aerh.employee_id AND c.org_id=aerh.org_id ) office_home_flag, aerh.report_header_id AS report_header_id_key, aerh.org_id AS org_id_key, gcc.code_combination_id AS code_combination_id_key, hou.organization_id AS organization_id_key, taerl.reprocess_counter AS reprocess_counter, row_number () OVER (PARTITION BY taerl.report_line_id ORDER BY db_txn_seq_no DESC NULLS LAST) srlno FROM  FXODS_MODEL_ITSS_AP.j_R12_300_Stg_PFL_EXPENSE_REPORT_DB_Ld taerl LEFT JOIN ITSS_AP.ods_ap_expense_rpt_headers_all aerh ON taerl.report_header_id = aerh.report_header_id AND (aerh.source <> 'NonValidatedWebExpense' OR aerh.workflow_approved_flag IS NULL) AND aerh.source <> 'Both Pay' AND taerl.org_id = aerh.org_id LEFT JOIN ITSS_AP.ods_gl_code_combinations gcc ON taerl.code_combination_id = gcc.code_combination_id LEFT JOIN ITSS_AP.ods_hr_operating_units hou ON hou.organization_id = aerh.org_id LEFT JOIN ITSS_AP.ods_per_all_people_f pap ON aerh.employee_id = pap.person_id AND (pap.effective_end_date = to_date ('31-Dec-4712' ,'DD-Mon-YYYY') OR pap.effective_end_date IS NULL ) WHERE NOT EXISTS (SELECT aerp.parameter_id FROM ITSS_AP.ods_ap_expense_rpt_params_all aerp, ITSS_AP.ods_ap_expense_reports_all aer WHERE aer.report_type = 'Seeded Personal Expense' AND aerp.expense_type_code = 'PERSONAL' AND aer.expense_report_id = aerp.expense_report_id AND taerl.web_parameter_id = aerp.parameter_id) AND ((taerl.itemization_parent_id IS NULL) OR (taerl.itemization_parent_id <> -1) ) ) mainq where srlno = 1)
